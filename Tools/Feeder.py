from dataSpout import dataSpout  # feeder inherits from dataspout
...
class Feeder(dataSpout):
    ...
    def mctTimeDistanceEnrichment(self, dataSpout):
        distanceList = dataSpout.getDistanceList(
            'inbound_airport', 'board_point')
        ...
        dataSpout.setUTCDateTimes(
            'inbound_airport', 'inbound_arrival_datetime', 'UTC_inbound_arrival_datetime')
        dataSpout.setUTCDateTimes(
            'board_point', 'departure_datetime', 'UTC_departure_datetime')

        dataSpout.setDateTimeDifferences(
    ...
    def loadTrainingData(self):
        #first lets get the training data
        dataflow = Feeder('C:\\Users\\Jo Ming\\Documents\\AirevoWorkspace\\AirevoCode-Space\\NoShowModel\\CSVFiles\\CSVFiles2.csv' , 2500000)
        fullChord = dataflow.getAllTableFields()
        print(fullChord)
