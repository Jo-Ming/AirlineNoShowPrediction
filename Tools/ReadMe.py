...
def showingTheFeeder(): #feeder allows the user to state a location outside the channel bank.
    ...
    myFeeder = Feeder(channel = 'TheQuantileWorld\mct_full.csv', streamSize = 10)
    ...
    myFeeder.setDataStream(myFeeder.getAllTableFields()) 
...
def showDataVisualisating():
    ...
    dataVis = dataVisualiser('savedModels\example2ModelSave\data\dataflow.csv') #smaller dataset for easier visualisation
...
def evaluatingAModel():
    ...
    #make an evaluator
    modelToEvaluate = Model("ourFreshModel", "savedModels\exampleSave\modelexampleSave")

    #next we need a dataflow, and labels 
    feederTool = Feeder('pnrSegmentPassenger', streamSize = 'max')
...
def comparing2Models():
    ...
    secondModel = Model("slimShadyModel", "savedModels\slimShadyModel\modelslimShadyModel") #The slim NN architecture trained with some features kept in the dark!
