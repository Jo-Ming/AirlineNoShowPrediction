...
from Tools.spoutHelperFunctions import *
import csv
import os
...
class dataSpout():
        ...
        def __init__(self, channel, streamSize):
                try:
                        ...
                        print("Unable To set channel. Constructing Empty Dataspout.")
                        self.dataStream = {}

        def setTableNodeFromEntryList(self, entryList):
                _tableNode = tableNode("Made from entryList.")
                _tableNode.setEntriesFromList(entryList)
        ...
        def setDataStream(self, chordKey):
                try:
                        ...
                        for key in chordKey:
                                ...
                                if (column[0] == None):
                                        print('Warning. ' +  str(key) + ' is returning None. please check field name matches Dictionary Key')
                                self.addData(key, column)

                        print("data stream set.")
                except: 
                        print("Error. Failed To set DataStream. Check to see if features training chord recieved matched the names of the features in the entry dictionary keys.")
        ...
        def typeCastChordToInt(self, chord):
                for key in chord:
                        ...
                        for element in strand:
                                try:
                                        if (element == '' or element == 'NULL'): #set any null values to 0
                                                output.append(0)
                                        else: 
        ...
        def nullToZeroChord(self, chord):
                try:
                        ...
                        print("Error: Unable to convert " + str(key) + " strand Nulls to 0.")
        ...
        def nullToZero(self, field):
                ...
                if containsNull:
                        print(str(field) + "Contains Null/empty value(s)")
