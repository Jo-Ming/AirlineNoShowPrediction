...
def demo_execution():
    ...
    #get some data to test the model on.
...
class noShowModel(Model):
    ...
    def shapDemo(self):
        ...
        dataflow, labels, chord = modelFeeder.loadTrainingData() #get dataflow and targetField and applies some data enrichment
        noShowModel = Model("noshowModel", "model\\97.8\\model")
        ...
        #shapData = np.array(dataflow[:42]) #for shap alone we could use pandas.DataFrame() however, keras only takes numpy arrays.
        explainer = shap.KernelExplainer(noShowModel.model,shapData)#make our explainer
